...
// PostHog project key comes from the environment. Never hard-code it.
const posthogApiKey = process.env.POSTHOG_API_KEY;
const posthogHost = process.env.POSTHOG_HOST || 'https://eu.i.posthog.com';

// Client-side PostHog loader. Empty when no key is set, so the page stays clean.
const posthogSnippet = posthogApiKey
  ? `
        <script>
            ...
            posthog.init(${JSON.stringify(posthogApiKey)}, { api_host: ${JSON.stringify(posthogHost)}, person_profiles: 'identified_only' });
        </script>`
  : '';
...
app.get('/', (req, res) => {
  res.send(`
    ...
    <head>
        ...
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">${posthogSnippet}
        <style>
    ...
    <body>
        ...
        <script>
            const track = (event, props) => window.posthog && posthog.capture(event, props);
            ...
            document.getElementById('uploadForm').addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (result.success) {
                        track('product_import_completed', { products_processed: result.productsProcessed });
                        statusDiv.innerHTML = \`
                            <div class="success">
                        ...
                        track('product_import_failed', { reason: 'server_error' });
                        statusDiv.innerHTML = \`<div class="error">❌ Error: \${result.error}</div>\`;
                    }
                } catch (error) {
                    track('product_import_failed', { reason: 'network_error' });
